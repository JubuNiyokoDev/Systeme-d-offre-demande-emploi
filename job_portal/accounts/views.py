...
from .forms import CustomUserCreationForm, LoginForm,CustomUserChangeForm,CustomPasswordChangeForm
from django.contrib import messages
from .models import CustomUser
from django.contrib.auth import logout,update_session_auth_hash
from django.shortcuts import get_object_or_404
from jobs.models import JobApplication,JobOffer
...
def toggle_ban_user(request, user_id):
    ...
    if target_user.is_staff or target_user.is_superuser:
        messages.error(request, "Impossible de bannir un administrateur ou un membre du staff.")
        return redirect('accounts:user_list')
    ...
    if target_user.is_banned:
        ...
        messages.success(
            request, 
            f"L'utilisateur {target_user.username} a été débanni avec succès."
        )
    else:
        target_user.ban_user()
        messages.success(
            request, 
            f"L'utilisateur {target_user.username} a été banni avec succès."
        )
...
def profile(request):
    ...
    if request.user.is_recruiter:
        ...
        published_jobs = JobOffer.objects.filter(
