    ...
    def handle(self, *args, **kwargs):
        try:
            ...
            if created:
                ...
                user.save()
                self.stdout.write(
                    self.style.SUCCESS(f"Utilisateur test créé: {user.username}")
                )
            else:
                self.stdout.write(
                    self.style.WARNING(f"Utilisateur test existe déjà: {user.username}")
                )
            ...
            for i in range(5):
                ...
                if created:
                    ...
                    self.stdout.write(
                        self.style.WARNING(f"Offre existe déjà: {job.title}")
                    )
