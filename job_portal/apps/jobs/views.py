...
def available_jobs(request):
    ...
    if location and location != "Tous les lieux":
        ...
        )  # Utilisation de iexact pour une correspondance exacte insensible à la casse
...
def job_create(request):
    if not (request.user.is_recruiter or request.user.is_staff):
        messages.error(
            request, "Seuls les recruteurs peuvent créer des offres d'emploi."
        )
    ...
    if request.method == "POST":
        ...
        form = JobOfferForm()
    return render(
        request, "jobs/job_form.html", {"form": form, "title": "Créer une offre"}
    )
...
def job_edit(request, job_id):
    ...
    if not (
        ...
        or job.publisher == request.user
    ):
        messages.error(
            request, "Vous n'avez pas la permission de modifier cette offre."
        )
        return redirect("jobs:available_jobs")
...
def apply_to_job(request, job_id):
    if request.user.is_recruiter:
        messages.error(
            request, "Les recruteurs ne peuvent pas postuler aux offres d'emploi."
        )
        return redirect("jobs:available_jobs")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            application.status, _ = ApplicationStatus.objects.get_or_create(
                name="Pending"
            )
            application.save()
    ...
    return render(
        request, "jobs/apply.html", {"form": JobApplicationForm(), "job": job}
    )
...
def my_applications(request):
    ...
    applications = JobApplication.objects.filter(
        Q(job__title__icontains=query), applicant=request.user
    ).order_by("-applied_at")
    return render(
        request,
        "jobs/my_applications.html",
        {"applications": applications, "query": query},
    )
...
def job_delete(request, job_id):
    ...
    if not (
        ...
        messages.error(
            request, "Vous n'avez pas la permission de supprimer cette offre."
        )
        return redirect("jobs:available_jobs")
...
def update_application_status(request, application_id):
    ...
    application = get_object_or_404(
        JobApplication, id=application_id, job__publisher=request.user
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.success(
                request, "Statut de la candidature mis à jour avec succès."
            )
